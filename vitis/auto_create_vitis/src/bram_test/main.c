...
int bram_read_write()
{
	...
	for(i = BRAM_BYTENUM*Start_Addr; i < BRAM_BYTENUM*(Start_Addr + Len); i += BRAM_BYTENUM) {
		...
		printf("Write data to address %d\t, data is %d\t\n", i, Write_Data);
		Write_Data += 1;
	}
//	for(i = BRAM_BYTENUM*Start_Addr ; i < BRAM_BYTENUM*(Start_Addr + Len) ; i += BRAM_BYTENUM)
//	{
...
//	PL_BRAM_CTRL_mWriteReg(PL_RAM_BASE, PL_RAM_ST_ADDR , BRAM_BYTENUM*Start_Addr) ;
	...
	//Set write address
	PL_BRAM_CTRL_mWriteReg(PL_RAM_BASE, PL_RAM_INIT_DATA , BRAM_BYTENUM*Write_Addr) ;
...
void IntrHandler(void *CallbackRef)
{
	...
	for(i = BRAM_BYTENUM*Write_Addr; i < BRAM_BYTENUM*(Write_Addr + Len) ; i += BRAM_BYTENUM)
	{
